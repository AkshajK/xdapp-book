...
export async function deploy(chain: string){
    ...
    exec(
        `cd chains/evm && forge build && forge create --legacy --rpc-url ${rpc} --private-key ${privateKey} src/xGroupChat.sol:xGroupChat && exit`,
        (err, out, errStr) => {
            if (err) {
...
export async function registerApp(src:string, target:string){
    ...
    const messenger = new ethers.Contract(
        ...
        JSON.parse(
            fs
                .readFileSync(
                    "./chains/evm/out/xGroupChat.sol/xGroupChat.json"
                )
                .toString()
        ).abi,
        signer
...
export async function sendMsg(src:string, msg){
    const srcNetwork = config.networks[src];
    let srcDeploymentInfo;
    try{
        srcDeploymentInfo = JSON.parse(fs.readFileSync(`./deployinfo/${src}.deploy.json`).toString());
    ...
    const tx = await (await messenger.sendMsg(msg.senderChain, msg.senderName, msg.senderAddress, msg.message)).wait();
...
export async function getMessages(src:string){
    const srcNetwork = config.networks[src];
    let srcDeploymentInfo;
    ...
    return await messenger.getMessages();
}  